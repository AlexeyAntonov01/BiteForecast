...
class BiteForecastManager:
    ...
    def calc_data(self, weather: WeatherSchema) -> dict:
        ...
        for key, values in forecast.fish.items():
            ...
            for i, hour in enumerate(hours[:13]):

                temperature_weight = self.calc_weight_distribution_gauss(
                    values.temperature.min, values.temperature.max,
                    hour.temperature,
                    values.temperature.tolerance_min, values.temperature.tolerance_max
                )

                pressure_weight = self.calc_weight_distribution_gauss(
                    values.pressure.min, values.pressure.max,
                    hour.pressure,
                    values.pressure.tolerance_min, values.pressure.tolerance_max
                )

                windSpeed_weight = self.calc_weight_distribution_gauss(
                    values.windSpeed.min, values.windSpeed.max,
                    hour.windSpeed,
                    values.windSpeed.tolerance_min, values.windSpeed.tolerance_max
                )

                ideal_center = (values.windAngle.min + values.windAngle.max) / 2
                diffAngle = self.get_shortest_angle_distance(hour.windAngle, ideal_center)
                ...
                humidity_weight = self.calc_weight_distribution_gauss(
                    values.humidity.min, values.humidity.max,
                    hour.humidity,
                    values.humidity.tolerance_min, values.humidity.tolerance_max
                )

                prec_weight = self.calc_weight_distribution_gauss(
                    values.prec.min, values.prec.max,
                    hour.prec,
                    values.prec.tolerance_min, values.prec.tolerance_max
                )

                time_weight = self.calc_best_time_of_day(hour.time, values.timeOfDay.hour, values.timeOfDay.weights)

                cloudiness_weight = self.calc_weight_penalty(hour.cloudiness, values.cloudiness_optimal, values.cloudiness_penalty)

                prec_type_weight = self.calc_weight_penalty(hour.precType, values.prec_type_optimal, values.prec_type_penalty)
                ...
                weight_dict_result[key]['hourly'][i] = {
                    ...
                    'raw_pressure': hour.pressure,
                    'raw_temperature': hour.temperature,
                    'raw_windSpeed': hour.windSpeed,
                }
    ...
    def calc_dynamic_delta(self, pressure_delta: float, windSpeed_delta: float, temperature_delta: float, forecast: BiteForecastIdeal, fish_index: str) -> tuple:
